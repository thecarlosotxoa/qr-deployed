...
CORS(
    ...
    resources={
        r"/*": {"origins": "http://<your_frontend_domain_or_IP>:<frontend_port>"}
    },
)

app.config.update(
    SESSION_COOKIE_SECURE=False,             # Enable this if your app uses HTTPS; otherwise, False
    ...
    SESSION_COOKIE_DOMAIN=None,             # Remove 'localhost' so it defaults to the EC2 public domain
    PERMANENT_SESSION_LIFETIME=timedelta(minutes=30),
)

# Database configuration
DATABASE_CONFIG = {
    'dbname': 'qr_final',               # Database name in RDS
    'user': 'postgres',                 # RDS username
    'password': 'your_RDS_password',    # RDS password / use .env variable
    'host': 'your_RDS_endpoint',        # RDS endpoint
